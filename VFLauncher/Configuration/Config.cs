...
namespace Configuration
{
    public class Config
    {
        ...
        public Config(string config)
        {
            ...
            {
                //Log.Error("{0} doesn't exist!", config);
                Environment.Exit(0);
            }
            else
        ...
        public T Read<T>(string name, T value, bool hex = false)
        {
            ...
            {
                //Log.Error("Error in {0} in line {1}", ConfigFile, lineCounter);
            }
            ...
            if (typeof(T) == typeof(bool))
            {
                ...
                {
                    //Log.Error("Error in {0} in line {1}", ConfigFile, lineCounter);
                    //Log.Error("Use default value for boolean config option: {0}. Default: {1}", name, value);
                }
            }
